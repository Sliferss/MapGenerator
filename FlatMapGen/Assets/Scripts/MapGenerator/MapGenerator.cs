...
public static class MapGenerator
{
    ...
    private static void AddForest(MapGraph graph)
    {
        foreach (var node in graph.nodesByCenterPosition.Values)
        {
            if (node.nodeType == MapGraph.MapNodeType.Grass)
            {
                ...
                {
                    if (neighbor.nodeType == MapGraph.MapNodeType.Grass || neighbor.nodeType == MapGraph.MapNodeType.Forest)
                    {
                        if (node.GetElevation() > 3f && node.GetElevation() < 9f)
                        {
                            node.nodeType = MapGraph.MapNodeType.Forest;
                        }
                        if (node.GetElevation() > 8f && node.GetElevation() < 17f)
                        {
